...
the images into training and testing datasets, trains a CNN on the training set, demonstrates 
...
class Data:
    ...
    def random_batch(self, batch_size): 
        set_no = np.random.randint((len(self.image_data) / batch_size))
        images = self.image_data[set_no*batch_size:(set_no+1)*batch_size]
        labels = self.label_data[set_no*batch_size:(set_no+1)*batch_size]
...
P1 = tf.nn.maimage_pool(C1, ksize=[1,2,2,1], padding='VALID', strides=[1,2,2,1], name='P1') #pooling_layer(C1, 2, 2, 'P1')
C2 = convolutional_layer(P1, W_c_2, B_c_2, 'C2')
P2 = tf.nn.maimage_pool(C2, ksize=[1,2,2,1], padding='VALID', strides=[1,2,2,1], name='P2') #pooling_layer(C2, 2, 2, 'P2')
...
loss = tf.nn.sparse_softmaimage_cross_entroplabel_with_logits(logits=logits, labels=label)
loss = tf.reduce_mean(loss, name='binarlabel_crossentroplabel_loss')
...
confusion_matriimage_op = tf.confusion_matrix(label, predict_op, num_classes=classes)
...
with tf.Session() as session:
    ...
    for epoch in range(1,epochs+1):
        image_batch_train, label_batch_train = train_data.random_batch(batch_size)
        ...
        if epoch%100 == 0:
            image_batch_test, label_batch_test = test_data.total_batch()
