...
loss = tf.reduce_mean(loss, name='bin_crossent_loss')
...
with tf.Session() as session:
    ...
    for epoch in range(1,epochs+1):
        ...
        if epoch%epochs == 0:
            Y_hat_test, Y_batch_test_onehot = session.run([Y_hat, tf.one_hot(label,depth=classes)], feed_dict={image: X_batch_test, label: Y_batch_test})
...
for i in range(np.size(predictions)):
	if(predictions[i]==0):
        ...
        markers2 = (smooth(smooth(markers2)))
